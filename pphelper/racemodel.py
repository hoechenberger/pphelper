...
Provides
--------
 - ``get_percentiles_from_raw_rts`` : Assess cumulative distribution functions from response time data and calculate the race model assumptions.
 - ``get_percentiles_from_dataframe`` : Assess cumulative distribution functions from response time data and calculate the race model assumptions.
...
def get_percentiles_from_raw_rts(rt_a, rt_b, rt_ab, num_percentiles=10,
    ...
    >>> racemodel.get_percentiles_from_raw_rts(C['x'], C['y'], C['z'])
                A      B          AB         A+B
    p
    ...
    if names is None:
        ...
        try:
            ...
            raise ValueError('Please supply the column names as a list '
                             'in get_percentiles_from_raw_rts().')
...
def plot_cdfs(data, percentile_index='p', colors=None, outfile=None):
    ...
    ``get_percentiles_from_raw_rts`` or or ``get_percentiles_from_dataframe``.
...
def get_percentiles_from_dataframe(data, rt_column='RT',
    ...
    Notes
    -----
    This function internally calls ``get_percentiles_from_raw_rts``. Please
    see this function to find out about additional optional keyword
    arguments.

    See Also
    --------
    get_percentiles_from_raw_rts
    ...
    result = get_percentiles_from_raw_rts(rt_a, rt_b, rt_ab,
